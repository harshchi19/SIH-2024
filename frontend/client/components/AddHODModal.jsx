...
import { ADD_AUTH_USER } from "@/utils/constants";
...
const AddHODModal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(ADD_AUTH_USER, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ email, password, userType: "HOD" }),
      });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className="bg-black">Add HOD</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add New HOD</DialogTitle>
          <DialogDescription>
            Create a new HOD account by providing an email and password
          </DialogDescription>
        </DialogHeader>
