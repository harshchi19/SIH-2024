import { decryptSection, encryptSection, generateHashedData, generateKeyAndIV } from "../helper/security.helper.js";
...
export const getPreTherapyUserById = async (req, res, next) => {
    ...
    try {
        ...
        const patientData = await Patient.findOne({ patient_id_hash: hashedPatientId }).select("_id");

        const existingEntry = await PreTherapy.findOne({ patient_id: patientData }).select("_id");
...
export const uploadPreTherapy = async (req, res, next) => {
    ...
    try {
        ...
        const existingEntry = await PreTherapy.findOne({ case_no_hash: hashedCaseNo }).select("_id");

        if (existingEntry) {
            return res.status(400).json({ message: "usr-alr-ext" });
        }

        const existingPatient = await Patient.findOne({ case_no_hash: hashedCaseNo }).select("name age sex date_of_assignment");
        ...
        const patientData = {
            ...
            date_of_assignment: Object.fromEntries(existingPatient.date_of_assignment)
        }
        ...
        const encryptedPatientDetails = encryptSection(encryptPatientDetails, key, iv);
        const encryptedFindings = encryptSection(findings, key, iv);

        console.log(encryptPatientDetails, encryptedFindings)
