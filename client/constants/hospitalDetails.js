export const hospitalDepartments = [
  "Speech and Language Therapy",
  "Otolaryngology (ENT)",
  "Neurology",
  "Pediatrics",
  "Pulmonology",
  "Gastroenterology",
  "Psychiatry",
  "Rehabilitation",
  "Oncology",
  "Cardiology",
  "Geriatrics",
  "Audiology",
  "Orthopedics",
];

export const qualificationsOptions = [
  "B.Sc. in Speech and Language Therapy",
  "M.Sc. in Speech and Language Therapy",
  "Ph.D. in Speech and Language Pathology",
  "Diploma in Speech and Language Therapy",
  "Certification in Speech and Language Pathology",
  "M.D. (Doctor of Medicine)",
  "M.B.B.S. (Bachelor of Medicine, Bachelor of Surgery)",
  "Clinical Fellowship in Speech and Language Therapy",
  "Board Certification in Speech-Language Pathology",
  "Post-Graduate Certificate in Audiology",
  "M.D. in Otolaryngology",
  "Certification in Audiology and Hearing Sciences",
  "Ph.D. in Neurological Sciences",
  "M.D. in Neurology",
  "Diploma in Child Health (DCH)",
  "M.D. in Pediatrics",
  "Certification in Pediatric Advanced Life Support (PALS)",
  "Certification in Pulmonary Rehabilitation",
  "Certification in Hepatology",
  "M.D. in Gastroenterology",
  "Ph.D. in Clinical Psychology",
  "Diploma in Psychiatry (DPM)",
  "Certification in Cognitive Behavioral Therapy (CBT)",
  "Diploma in Physical Medicine and Rehabilitation",
  "Certification in Cancer Genomics",
  "Certification in Echocardiography",
  "Certification in Interventional Cardiology",
  "Certification in Clinical Audiology",
  "Certification in Sports Medicine",
];
